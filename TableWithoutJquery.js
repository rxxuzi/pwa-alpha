...
function refreshImage() {
    ...
    /**
     * ランダムなクエリパラメータを追加する主な目的は、キャッシュの問題を回避すること。
     * ブラウザは同じURLを持つリソースをキャッシュし、次回同じリソースが要求された場合にはキャッシュから取得するため
     * 動画が更新されても古いバージョンが表示されることがある。
     * しかし、クエリパラメータをランダムに生成することで、URLが異なるため、ブラウザは新しいリソースとして動画をダウンロードして再生する。
     *
     * 例えば、${url}?v=${Math.random()}のMath.random()は、0から1の間でランダムな数値を返すJavaScriptの組み込み関数です。
     * これにより、クエリパラメータがランダムな値で更新され、キャッシュの問題を回避することができます。
     * @type {string}
     */
